...
class Home extends MY_Controller 
{
	...
	*----------------------------------------------
	*/
	public function fetchMarksheetTable($subId = null,$studentId=null)
	{
		if($subId && $studentId) {			
			$subData = $this->model_subject->fetchSubjectData($subId);
			$scheduleData = $this->model_schedule->fetchScheduleData($subId);
			// $classData = $this->model_classes->fetchClassData($classId);
			$table = '

			<div class="well">
				Tên môn học : '.$subData['name'].'
			</div>
			    ...
			    <tbody>';
			    	if($scheduleData) {
			    		foreach ($scheduleData as $key => $value) {			  
			    			...
			    			$string=(isset($value['id'])?'Đã đăng ký':'Chưa đăng ký');
			    			$string2=(!isset($value['id'])?'Đăng ký':'Hủy đăng ký');
			    			$strdata=(!isset($value['id'])?'#addTimetableModal':'#removeTimetableModal');
			    			$stclick=(!isset($value['id'])?'addTimetable':'removeTimetable');
							$button = '<div class="btn-group">
							<button class="btn btn-primary" data-toggle="modal" data-target="'.$strdata.'" onclick="'.$stclick.'('.$value['schedule_id'].','.$studentId.','.$subId.')">'.$string2.'
							</button>	
					
							</div>';

				    		$table .= '<tr>
				    			<td>'.$value['name'].'-N'.$value['schedule_id'].'</td>
	...
	*/
	public function create($scheduleId = null, $studentId = null)
	{
		if($scheduleId && $studentId) {
			$create = $this->model_timetable->create($scheduleId,$studentId);
			if($create == true) {
	...
	*----------------------------------------------
	*/
	public function fetchUpdateMarksheetTable($subId = null, $studentId = null)
	{
		if($subId && $studentId) {			
			$subData = $this->model_subject->fetchSubjectData($subId);
			$scheduleData = $this->model_schedule->fetchScheduleData($subId);
	...
	*/
	...
	{
		if($scheduleId && $studentId) {
			$remove = $this->model_timetable->remove($scheduleId,$studentId);
			if($remove === true) {
				$validator['success'] = true;
			...
			$validator['html'] = '<div class="panel panel-default">		  	
				...
				<div class="panel-body">		  
					<div class="well well-sm">
						...
						Tên bảng điểm : '.$marksheetNameData['marksheet_name'].' <br />
